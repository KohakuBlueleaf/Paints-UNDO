...
@torch.no_grad()
def sample_dpmpp_2m(
    model, x, sigmas, extra_args=None, callback=None, progress_tqdm=None
):
    """DPM-Solver++(2M)."""
...
class KModel:
    def __init__(
        self, unet, timesteps=1000, linear_start=0.00085, linear_end=0.012, linear=False
    ):
        if linear:
            betas = torch.linspace(
                linear_start, linear_end, timesteps, dtype=torch.float64
            )
        else:
            betas = (
                torch.linspace(
    ...
    def __call__(self, x, sigma, **extra_args):
        ...
        eps_positive = self.unet(
            x_ddim_space, t, return_dict=False, **extra_args["positive"]
        )[0]
        eps_negative = self.unet(
            x_ddim_space, t, return_dict=False, **extra_args["negative"]
        )[0]
...
class KDiffusionSampler:
    ...
    def __call__(
        ...
        if same_noise_in_batch:
            ...
            initial_latent = initial_latent.repeat(batch_size, 1, 1, 1).to(
        ...
        negative_prompt_embeds = negative_prompt_embeds.repeat(batch_size, 1, 1).to(
            device
        )
